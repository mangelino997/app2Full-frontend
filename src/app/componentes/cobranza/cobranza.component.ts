...
export class CobranzaComponent implements OnInit {
  ...
  //Define la lista de resultados de busqueda de clientes
  ...
  public ventasComprobantesSeleccionados = new SelectionModel<any>(true, []);
  ...
  constructor(private servicio: CobranzaService, private toastr: ToastrService,
    private loaderService: LoaderService, private appService: AppService,
    private clienteServicio: ClienteService, private modelo: Cobranza,
    private dialog: MatDialog, private ventaComprobanteService: VentaComprobanteService) { }
  ...
  public estanTodosSeleccionados(): boolean {
    let numSeleccionado = this.ventasComprobantesSeleccionados.selected.length;
    let numFilas = this.ventasComprobantes.data.length;
  ...
  public alternarSeleccion(): void {
    this.estanTodosSeleccionados() ?
      this.ventasComprobantesSeleccionados.clear() :
      this.ventasComprobantes.data.forEach(row => this.ventasComprobantesSeleccionados.select(row));
  ...
  public checkboxLabel(row?: any): string {
    ...
    return `${this.ventasComprobantesSeleccionados.isSelected(row) ? 'deselect' : 'select'} row ${row.position + 1}`;
  }
  //Al seleccionar un checkbox
  public seleccionarCheckbox(row): void {
    this.ventasComprobantesSeleccionados.toggle(row);
    //Establece el total de items seleccionados e importe total seleccionado
    this.calcularTotalItemsEImporteTotalSeleccionado();
    //Calcula importe Pendiente de Integrar
    this.formularioIntegrar.get('pendienteIntegrar').setValue(this.calcularPendienteIntegrar());;
  }
  //Obtiene una lista de ventas comprobantes por empresa y proveedor
  public listarVentasPorClienteYEmpresa(): void {
    this.loaderService.show();
    let cliente = this.formulario.get('cliente').value;
    let empresa = this.appService.getEmpresa();
    this.ventaComprobanteService.listarPorClienteYEmpresa(cliente.id, empresa.id).subscribe(
      res => {
        this.ventasComprobantes = new MatTableDataSource(res.json());
        this.ventasComprobantes.sort = this.sort;
        this.ventasComprobantes.data.length == 0 ? this.toastr.warning("El cliente no tiene comprobantes asignados.") : '';
  ...
  private calcularTotalItemsYTotalDeuda(): void {
    this.formularioTotales.get('totalItems').setValue(this.ventasComprobantes.data.length);
    let deuda = 0;
  ...
  public calcularTotalItemsEImporteTotalSeleccionado(): void {
    this.formularioTotales.get('itemsImporte').setValue(this.ventasComprobantesSeleccionados.selected.length);
    let deuda = 0;
    this.ventasComprobantesSeleccionados.selected.forEach((elemento) => {
      if (elemento.tipoComprobante.id == 3 || elemento.tipoComprobante.id == 28) {
        deuda -= elemento.importeSaldo;
      } else {
        deuda += elemento.importeSaldo;
  ...
  private abrirDialogo(componente, formularioDialogo): void {
    ...
    const dialogRef = this.dialog.open(componente, {
      ...
      maxWidth: '95%',
      data: {
        idCliente: this.formulario.get('cliente').value.id,
