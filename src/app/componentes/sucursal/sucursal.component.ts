...
export class SucursalComponent implements OnInit {
  ...
  //Constructor
  constructor(private servicio: SucursalService, private pestaniaService: PestaniaService,
  ...
  //Funcion para listar por barrios nombre
  listarBarriosPorNombre = (text$: Observable<string>) => text$.pipe(
    map(term => term.length < 2 ? [] : this.barrioServicio.listarPorNombre(term))
  )
  formatearBarrios = (x: {nombre:string}) => x.nombre;
  //Funcion para listar por localidades nombre
  ...
  formatearLocalidades = (x: {nombre:string, provincia:any}) => x.nombre + ' - '
