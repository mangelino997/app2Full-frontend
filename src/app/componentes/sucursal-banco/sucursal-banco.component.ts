...
export class SucursalBancoComponent implements OnInit {
  ...
  constructor(private servicio: SucursalBancoService, private pestaniaService: PestaniaService,
    private bancoServicio: BancoService, private appComponent: AppComponent, private toastr: ToastrService) {
  ...
  buscar = (text$: Observable<string>) => text$.pipe(
    map(term => term.length < 2 ? [] : this.servicio.listarPorNombreBanco(term))
  )
  formatear = (x: {nombre:string}) => x.nombre;
  //Funcion para listar por nombre
  listarBancosPorNombre = (text$: Observable<string>) => text$.pipe(
    map(term => term.length < 2 ? [] : this.bancoServicio.listarPorNombre(term))
  )
  formatearBancos = (x: {nombre:string}) => x.nombre;
