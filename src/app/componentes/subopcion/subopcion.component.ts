...
export class SubopcionComponent implements OnInit {
  ...
  //Funcion para listar por nombre
  listarSubmodulosPorNombre = (text$: Observable<string>) => text$.pipe(
    map(term => term.length < 2 ? [] : this.submoduloServicio.listarPorNombre(term))
  )
  formatearSubmodulos = (x: {nombre:string, modulo:any}) => x.nombre + ' - ' + x.modulo.nombre;
