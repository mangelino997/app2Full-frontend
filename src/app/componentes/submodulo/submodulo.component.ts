...
export class SubmoduloComponent implements OnInit {
  ...
  //Funcion para listar por nombre
  listarModulosPorNombre = (text$: Observable<string>) => text$.pipe(
    map(term => term.length < 2 ? [] : this.moduloServicio.listarPorNombre(term))
  )
  formatearModulos = (x: {nombre:string}) => x.nombre;
